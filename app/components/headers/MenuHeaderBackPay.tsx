...
export default function MenuHeaderBackPay({
  restaurant,
  tableNumber,
  hidden = false,
}: MenuHeaderProps) {
  ...
  return (
    <header
      className={`transition-all duration-700 container mx-auto px-5 md:px-8 lg:px-10 pt-5 md:pt-7 lg:pt-9 z-10 ${
          ...
          <div className="flex items-center space-x-1">
            {remainingCount > 0 && (
              <div
                ...
                className="size-10 md:size-12 lg:size-14 bg-white rounded-full flex items-center justify-center text-black text-base font-medium border border-[#8e8e8e] shadow-sm cursor-pointer"
              >
                +{remainingCount}
              </div>
            )}
            {visibleParticipants.map((participant, index) => {
              ...
              return (
                <div
                  ...
                  onClick={() => setIsParticipantsModalOpen(true)}
                  className={`size-10 md:size-12 lg:size-14 rounded-full flex items-center justify-center text-white text-base font-medium border border-white shadow-sm cursor-pointer overflow-hidden ${!hasImage ? getAvatarColor(displayName) : ""}`}
                  style={{
