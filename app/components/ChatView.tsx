...
const MessageContent = ({ content, isStreaming, activeTool }: { content: string; isStreaming?: boolean; activeTool?: string | null }) => {
  ...
  if (isStreaming) {
    ...
    parts.forEach((part, i) => {
      if (part) {
        elements.push(<span key={`t${i}`} className="whitespace-pre-wrap">{renderBoldText(part)}</span>);
      }
      if (i < parts.length - 1) {
...
export default function ChatView({ onBack }: ChatViewProps) {
  ...
  const { messages, setMessages, sessionId, setSessionId, hasStartedChat, setHasStartedChat } = usePepper();
